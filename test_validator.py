...
class ValidatorTest(unittest.TestCase):
    ...
    def test_HoldTheLine(self):
        ...
        JCorrent = [303.8495134310999, 164.78849057293556, 113.97289336455995, 86.78895269391742,
        ...
        for i, nrob in enumerate(NoRobots):
            ...
            opt_loop = OptimizationLoop(testbedName, propertiesFILE=props)
            curCF = opt_loop.getJJ()[Tmax - 1]
            print("[Number of robots: {}] Average CF value: {} | Recorded CF value: {}".format(nrob, JCorrent[i], curCF))
            self.assertTrue(curCF >= JCorrent[i] * (1 - deviation) and curCF <= JCorrent[i] * (1 + deviation))

    def test_AdaptiveCoverage2D(self):
        testbedName = "AdaptiveCoverage2D"
        NoRobots = [5, 10, 15, 20]
        DesiredCF = [1964.9232312423503, 1436.8281919714111, 1174.6110007708967, 1024.7571168934032]
        deviation = 0.15  # 15%
        path = "testbeds/{}/Parameters.properties".format(testbedName)
        # Initial robot positions for each scenario:
        InitialRobotPos = []
        InitialRobotPos.append([
        ...
        for i, nrob in enumerate(NoRobots):
            ...
            props["noRobots"] = str(nrob)
            Tmax = int(props["noIter"])
            opt_loop = OptimizationLoop(testbedName, propertiesFILE=props, initialDecisions=InitialRobotPos[i])
            curCF = opt_loop.getJJ()[Tmax - 1]
            print("[Number of robots: {}] Average CF value: {} | Recorded CF value: {}".format(nrob, DesiredCF[i], curCF))
            self.assertTrue(curCF >= DesiredCF[i] * (1 - deviation) and curCF <= DesiredCF[i] * (1 + deviation))
